			...
			if(e.getClickedBlock().getType().equals(Material.CLAY) && e.getClickedBlock().getRelative(BlockFace.UP, 1).getType().equals(Material.WATER)) {
				...
				if(pans.contains(i)) {
					...
					if(num == 1) {
						...
						p.playSound(p.getLocation(), Sound.BLOCK_GRASS_BREAK, 1, 1.4f);
						
						p.sendMessage(preMessage + "You panned " + ChatColor.AQUA + amount + ChatColor.GOLD + " Gold Nuggets.");
					}
					p.playSound(p.getLocation(), Sound.BLOCK_GRASS_STEP, 1, 1.4f);
				...
				if(pans.contains(i)) {
					...
					if(num == 1) {
						...
						gpUpdate.setDisplayName(ChatColor.GOLD + "Gold Pan " + ChatColor.AQUA + "[ "+ uses +"/10 ]");
						i.setItemMeta(gpUpdate);
						p.playSound(p.getLocation(), Sound.BLOCK_SAND_BREAK, 1, 1.4f);
						
						p.sendMessage(preMessage + "You panned " + ChatColor.AQUA + amount + ChatColor.GOLD + " Gold Nuggets.");
					}
					p.playSound(p.getLocation(), Sound.BLOCK_SAND_STEP, 1, 1.4f);
				...
				return;
			}else if(e.getClickedBlock().getType().equals(Material.GRAVEL) && e.getClickedBlock().getRelative(BlockFace.UP, 1).getType().equals(Material.WATER)) {
				Block b2 = e.getClickedBlock();
				if(pans.contains(i)) {
					Random rand = new Random();
					int num = rand.nextInt(3);
					...
					if(num == 1) {
						...
						gpUpdate.setDisplayName(ChatColor.GOLD + "Gold Pan " + ChatColor.AQUA + "[ "+ uses +"/10 ]");
						i.setItemMeta(gpUpdate);
						p.playSound(p.getLocation(), Sound.BLOCK_GRAVEL_STEP, 1, 1.4f);
				...
				return;
			}else if(pans.contains(p.getItemInHand())) {
						p.sendMessage(preMessage + ChatColor.RED + "You must be looking at " + 
						ChatColor.GOLD + "dirt"
						+ ChatColor.RED + ", "
						+ ChatColor.GOLD + "clay"
						+ ChatColor.RED + ", "
						+ ChatColor.GOLD + "gravel"
						+ ChatColor.RED + ", or "
						+ ChatColor.GOLD + "sand"
						+ ChatColor.RED + " that is underwater.");
			}
